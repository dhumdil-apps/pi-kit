...
import { applyPatch, detectGit, patchFailureStatus, runAcceptanceChecks, runPackageChecks } from "./checkpoint.js";

describe("checkpoint scheduling and patch sequencing", () => {
	...
	it("does not apply a patch when the three-way preflight fails", async () => {
		const runner = vi.fn(async (_command: string, args: string[]) => ({ code: args.includes("--check") ? 1 : 0, stdout: "", stderr: "conflict" }));
		expect(await applyPatch(runner, "/tmp/change.patch")).toMatchObject({ ok: false, stage: "check" });
		expect(runner).toHaveBeenCalledTimes(1);
	});

	it("runs planner acceptance commands as targeted slice checks", async () => {
	...
	it("permits one sequential redispatch and then blocks", () => {
		expect(patchFailureStatus(1)).toBe("redispatched");
		expect(patchFailureStatus(2)).toBe("blocked");
	});
});
