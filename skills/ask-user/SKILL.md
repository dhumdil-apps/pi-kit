...
For a plan-approval confirmation, `options` is mandatory even though there is
only one choice: pass `{ "title": "Proceed" }` and leave freeform enabled for
revision feedback. Never ask a bare `Proceed?` question.
