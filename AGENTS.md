...
- Plan Mode is split into `index.ts`, `state.ts`, `ledger.ts`, `orchestration.ts`, and `checkpoint.ts`; keep pure logic out of the event-wiring closure when practical.
- The parent Plan Mode agent alone owns user decisions, todos, patch integration, commits, and acceptance.
- Preserve headless/subagent bypasses, destructive-only Permission Gate behavior, and the exact active-ledger worktree exception.
